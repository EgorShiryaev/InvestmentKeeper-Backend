...
import CreateInvestmentAccountData from '../../domain/entities/api_data/create_investment_account_data';
    ...
    handler: async (request, response) => {
      ...
      try {
        if (createInstrumentAccountData.type === InvestmentAccountType.IIA){
          const iiaIsAlreadyExists = !!(await repository.getAll(user.id)).find((item)=>item.type === InvestmentAccountType.IIA);
          if (iiaIsAlreadyExists){
            response.status(StatusCode.badRequest).json({ status: 'Individual Investment Account is already exist' });
            return;
          }
        }
